...
* 删除`3389`、`22`入方向的端口开放
* 点击快速创建规则
  * 勾选22、80、3306、3389、6379
  * 手动填写TCP：8080
  * 获取真实ip：通过http://www.net.cn/static/customercare/yourip.asp
    * ip填写至授权对象中
  * 点击确定
* 点击体验新版
  * 点击入方向手动添加
    * 端口范围：51110，51120，51130，51140
    * 授权对象：0.0.0.0/0（代表任意ip都可访问）
    * 点击保存

### 7.1.2 重置root密码

* 云服务器ECS —— 点击具体实例 —— 基本信息 —— 更多 —— 重置实例密码
  * 填写超长密码（大写字母，小写字母，特殊符号，大写字母，小写字母，标点符号，数字）
...
* SecureCRT ——New Session —— 下一步
  ...
  * 双击刚才创建的Session
    ...
    * Password输入刚才重置后的密码
      * 勾选Save password
      * 点击ok
  * 设置点击Options —— Session Options..
    * Terminal —— Emulation
      * 勾选ANSI Color
    * Appearance
      * 修改Character encoding为UTF-8
    * 点击ok
...
firewall-cmd --zone=public --add-port=51110/tcp --permanent
firewall-cmd --zone=public --add-port=51120/tcp --permanent
firewall-cmd --zone=public --add-port=51130/tcp --permanent
firewall-cmd --zone=public --add-port=51140/tcp --permanent
```
...
cd /usr/local && tar -zxvf nginx-1.16.1.tar.gz
...
cd /usr/local
# 上传
rz（mysql-5.7.29-el7-x86_64.tar.gz）
# 解压
...
```sql
CREATE DATABASE IF NOT EXISTS storage_dev DEFAULT CHARACTER SET utf8mb4 DEFAULT COLLATE utf8mb4_general_ci;
CREATE DATABASE IF NOT EXISTS storage_prod DEFAULT CHARACTER SET utf8mb4 DEFAULT COLLATE utf8mb4_general_ci;
GRANT ALL PRIVILEGES ON `storage_dev`.* TO 'storage_dev'@'%' IDENTIFIED BY 'storage_dev' WITH GRANT OPTION;
GRANT ALL PRIVILEGES ON `storage_prod`.* TO 'storage_prod'@'%' IDENTIFIED BY 'storage_prod' WITH GRANT OPTION;
FLUSH PRIVILEGES;
```
...
CREATE TABLE `t_advertisement` (
  ...
  `img_url` varchar(1024) DEFAULT NULL COMMENT '图片路径',
...
CREATE TABLE `t_advice` (
  `id` bigint(20) unsigned NOT NULL AUTO_INCREMENT COMMENT 'id',
  `user_id` bigint(20) unsigned NOT NULL COMMENT '手机端用户id',
  `title` varchar(64) NOT NULL COMMENT '标题',
  `content` varchar(4096) NOT NULL COMMENT '内容',
...
CREATE TABLE `t_box_info` (
  ...
  `box_unit_price` bigint(20) unsigned NOT NULL COMMENT '箱子积分单价（每月积分单价）',
  `used_by` bigint(20) unsigned DEFAULT NULL COMMENT '使用人',
  `is_used` tinyint(3) unsigned NOT NULL COMMENT '是否被使用（0：未使用；1：已使用）',
...
CREATE TABLE `t_order` (
  `id` bigint(20) unsigned NOT NULL AUTO_INCREMENT COMMENT 'id',
  `user_id` bigint(20) unsigned NOT NULL COMMENT '手机端用户id',
  `status` tinyint(3) unsigned NOT NULL COMMENT '订单状态（手机端——1：已下上门单；2：待收空箱；3：已收空箱；4：已发重箱；5：已存储；6：已下提取单；7：待收重箱；8：已收重箱；9：已发空箱；10：已完成）订单状态（后台端——1：待发空箱；2：已发空箱；3：送达空箱；4：待发重箱；5：已存储；6：待发重箱；7：已发重箱；8：送达重箱；9：待收空箱；10：已完成）',
  `order_code` bigint(20) unsigned NOT NULL COMMENT '订单编号（年月日+6位序列）',
  `order_name` varchar(64) NOT NULL COMMENT '订单名称',
  `empty_box_order_time` datetime NOT NULL COMMENT '空箱上门下单时间（订单状态为1-5，显示）',
  `heavy_box_order_time` datetime DEFAULT NULL COMMENT '重箱提取下单时间（订单状态为6-10，显示）',
  `empty_box_call_name` varchar(64) NOT NULL COMMENT '空箱上门姓名（订单状态为1-5，显示）',
  `empty_box_call_phone` varchar(64) NOT NULL COMMENT '空箱上门电话（订单状态为1-5，显示）',
  `empty_box_call_address` varchar(1024) NOT NULL COMMENT '空箱上门地址（订单状态为1-5，显示）',
  `empty_box_call_time` date NOT NULL COMMENT '空箱上门时间（年月日，订单状态为1-5，显示）',
  `empty_box_call_interval` varchar(64) NOT NULL COMMENT '空箱上门时段（如09:00-13:00，订单状态为1-5，显示）',
  `heavy_box_call_name` varchar(64) DEFAULT NULL COMMENT '重箱提取姓名（订单状态为6-10，显示）',
  `heavy_box_call_phone` varchar(64) DEFAULT NULL COMMENT '重箱提取电话（订单状态为6-10，显示）',
  `heavy_box_call_address` varchar(1024) DEFAULT NULL COMMENT '重箱提取地址（订单状态为6-10，显示）',
  `heavy_box_call_time` date DEFAULT NULL COMMENT '重箱提取时间（年月日，订单状态为6-10，显示）',
  `heavy_box_call_interval` varchar(64) DEFAULT NULL COMMENT '重箱提取时段（如09:00-13:00，订单状态为6-10，显示）',
  ...
  `version` bigint(20) unsigned NOT NULL COMMENT '版本号',
  `del_flag` char(1) NOT NULL COMMENT '删除标记（0：未删除；2：已删除）',
  `app_del_flag` tinyint(3) unsigned NOT NULL COMMENT '手机端删除标记（0：未删除；2：已删除）',
  `backend_del_flag` tinyint(3) unsigned NOT NULL COMMENT '后台端删除标记（0：未删除；2：已删除）',
  PRIMARY KEY (`id`),
  KEY `idx_user_id` (`user_id`),
...
CREATE TABLE `t_point_record` (
  `id` bigint(20) unsigned NOT NULL AUTO_INCREMENT COMMENT 'id',
  `user_id` bigint(20) unsigned NOT NULL COMMENT '手机端用户id',
  `way` tinyint(3) unsigned NOT NULL COMMENT '方式（1：注册赠送；2：活动获得；3：积分使用）',
...
-- 初始化-部门表数据
-- ----------------------------
INSERT INTO `sys_dept` VALUES ('100', '0', '0', 'neutech', '0', '王东', '15142815248', 'wangdong.neu@neusoft.com', '0', '0', 'admin', '2018-03-16 11:33:00', 'admin', '2020-05-02 17:09:25');
INSERT INTO `sys_dept` VALUES ('101', '100', '0,100', 'java', '1', 'java', '15888888888', 'java@qq.com', '0', '0', 'admin', '2018-03-16 11:33:00', 'admin', '2020-05-02 17:09:25');
INSERT INTO `sys_dept` VALUES ('103', '101', '0,100,101', '研发部门', '1', '若依', '15888888888', 'ry@qq.com', '0', '0', 'admin', '2018-03-16 11:33:00', 'ry', '2018-03-16 11:33:00');
INSERT INTO `sys_dept` VALUES ('105', '101', '0,100,101', '测试部门', '2', '若依', '15888888888', 'ry@qq.com', '0', '0', 'admin', '2018-03-16 11:33:00', 'ry', '2018-03-16 11:33:00');
INSERT INTO `sys_dept` VALUES ('113', '101', '0,100,101', '后台端部门', '3', '若依', '15888888888', 'ry@qq.com', '0', '0', 'admin', '2018-03-16 11:33:00', 'ry', '2018-03-16 11:33:00');
INSERT INTO `sys_dept` VALUES ('114', '101', '0,100,101', '手机端部门', '4', '若依', '15888888888', 'ry@qq.com', '0', '0', 'admin', '2018-03-16 11:33:00', 'ry', '2018-03-16 11:33:00');
...
create table sys_user (
  ...
  user_type         varchar(2)      default '00'               comment '用户类型（00系统用户；01后台端用户；02手机端用户）',
  ...
  `current_points`  bigint(20)     unsigned NULL               COMMENT '当前积分',
...
-- 初始化-岗位信息表数据
-- ----------------------------
insert into sys_post values(1, 'admin',  '管理员',    1, '0', 'admin', '2018-03-16 11-33-00', 'ry', '2018-03-16 11-33-00', '');
...
INSERT INTO `sys_role` VALUES ('3', '后台端角色', 'backend', '3', '1', '0', '0', 'admin', '2020-05-02 17:15:33', '', null, '后台端角色');
INSERT INTO `sys_role` VALUES ('4', '手机端角色', 'app', '4', '1', '0', '0', 'admin', '2020-05-02 17:15:55', '', null, '手机端角色');
...
http {
    ...
    server {
        ...
        location /prod-api/{
            ...
            proxy_pass http://localhost:51110/;
        }


        location /gitbook/{
            alias /usr/local/nginx/gitbook/_book/;
            index index.html;
        }
...
# 上传前端文件
cd /home/ruoyi/projects/ruoyi-ui
rz（dist.tar.gz）
# 解压
tar -zxvf dist.tar.gz
# 创建后端目录
mkdir -p /home/ruoyi/projects/ruoyi
# 创建上传目录
mkdir -p /home/ruoyi/uploadPath
# 创建gitbook目录
mkdir -p /usr/local/nginx/gitbook
cd /usr/local/nginx/gitbook
# 上传gitbook
rz（_book.tar.gz）
# 解压
...
* 修改一套属于自己的配置文件【application-test.yml】
...
spring:
  datasource:
    ...
    druid:
      # 主库数据源
      master:
        url: jdbc:mysql://localhost:3306/storage_prod?useUnicode=true&characterEncoding=utf8&zeroDateTimeBehavior=convertToNull&useSSL=true&serverTimezone=GMT%2B8
        username: storage_prod
        password: storage_prod
      # 从库数据源
      slave:
        # 从数据源开关/默认关闭
        enabled: false
...
* 上传ruoyi.jar到/home/ruoyi/projects/ruoyi下

```shell
cd /home/ruoyi/projects/ruoyi
rz（ruoyi.jar）
# 查看剩余内存
free -h
# 守护进程启动（根据服务器剩余内存，调整-Xms及-Xmx）
nohup java -jar -Xms1024m -Xmx1024m ruoyi.jar >/dev/null 2>&1 & 
# 如果关闭
ps -ef|grep java
netstat -anpt|grep 51110
# 杀死线程
kill 9 线程id
```
...
* Swagger：【公网ip】:51110/swagger-ui.html
* Gitbook：【公网ip】:51120/gitbook/
* 给到前端的地址：【公网ip】:51120